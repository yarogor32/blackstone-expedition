(() => {
 ...
 function say(hero,kind){
  ...
  if(now-lastAny<rules.partyCooldown||now-(spoken.get(hero.id)??-Infinity)<(critical?rules.criticalCooldown:rules.ordinaryCooldown))return;
  if(!critical&&Math.random()> (kind==='neutral'?rules.neutralChance:rules.ordinaryChance))return;
  const locale=window.EXPEDITION_LINES[document.documentElement.lang]||window.EXPEDITION_LINES.en;
  const lines=(locale[hero.id]||locale.default)[kind];if(!lines?.length)return;
  ...
  timers.set(hero.id,setTimeout(()=>bubble.remove(),rules.duration));
 }
 function impact(battle,hit){if(!hit)return;const a=battle.units.find(u=>u.id===hit.attacker),t=battle.units.find(u=>u.id===hit.target);
  ...
  if(t?.side==='party')say(t,hit.missed?'positive':hit.critical?'extremelyNegative':'negative');
 }
